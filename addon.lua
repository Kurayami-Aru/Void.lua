...
WindUI:Popup({
    Title = "Welcome to Zeryx Hub!",
    Icon = "info",
    Content = "This Script Is " .. gradient("Zeryx", Color3.fromHex("#000000"), Color3.fromHex("#ff0000")) .. " by Zeryx Team",
    Buttons = {
        {
            Title = "Cancel",
            Variant = "Secondary",
...
Window:EditOpenButton({
    Title = "Open Zeryx Hub",
    Icon = "zap",
    CornerRadius = UDim.new(0, 16),
    StrokeThickness = 2,
    Color = ColorSequence.new(Color3.fromHex("ff0000"), Color3.fromHex("000000")),
    Draggable = true,
})
...
local Tabs = {
    MainTab = Window:Tab({ Title = "Main", Icon = "swords" }),
    CustomTab = Window:Tab({ Title = "Customize", Icon = "cloud" }),
    DetectionTab = Window:Tab({ Title = "Detection", Icon = "shield" }),
...
Tabs.MainTab:Toggle({    
    Title = "Lobby AP",    
    Desc = "Automatically Parries in lobby",    
    Callback = function(state)    
		if state then
...
Tabs.MainTab:Slider({
    ...
    Desc = "Parries Needed To Activate Spam",
...
Tabs.PlayerTab:Slider({
    Title = "Strafe Speed",
    Desc = "",
    Value = { Min = 36, Max = 350, Default = 36 },
    Callback = function(value)
         StrafeSpeed = value
    end
})
...
Tabs.DetectionTab:Toggle({
    ...
    Desc = "Stop parry when turn on Infinity",
    Callback = function(value)
        if value then
...
Tabs.DetectionTab:Toggle({
    ...
    Desc = "Auto Parry when turn on SOF",
    Callback = function(value)
        if value then                               getgenv().SlashOfFuryDetection = value
            end
        end
    })
...
Tabs.DetectionTab:Toggle({
    Title = "Anti Phantom",
    Desc = "Stop parry when Phantom target you",
    Callback = function(value)
        if value then
...
Tabs.DetectionTab:Toggle({
    ...
    Desc = "Auto use ability when you miss parry",
    Callback = function(value)
        if value then
