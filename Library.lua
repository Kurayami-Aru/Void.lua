...
function Library:save_flags()
    ...
    writefile(`Moonlight/{game.GameId}.lua`, flags)
end

function Library:load_flags()
    if not isfile(`Moonlight/{game.GameId}.lua`) then Library.save_flags() return end
    local flags = readfile(`Moonlight/{game.GameId}.lua`)
...
mobile_button.Parent = container -- Assumes 'container' is defined elsewhere
...
-- Replaced ImageLabel with TextLabel for pixelated "Z"
...
Icon.Font = Enum.Font.Arcade -- Pixelated font
Icon.Text = "M"
Icon.TextColor3 = Color3.fromRGB(255, 255, 255) -- White text for visibility
Icon.TextSize = 20 -- Adjust size for pixelated look
Icon.TextStrokeTransparency = 0 -- Add stroke for better contrast
Icon.TextStrokeColor3 = Color3.fromRGB(0, 0, 0) -- Black stroke
...
function Module:create_keybind()
    ...
    KeyText.Size = UDim2.new(0, 29, 0, 15)
    KeyText.ZIndex = 2
    ...
    KeyText.Text = self.keycode and self.keycode.Name or "None"

    if not Library.Flags[self.flag] then
        Library.Flags[self.flag] = self.keycode and self.keycode.Name or "None"
    end
    ...
    Library:connect(UserInputService.InputBegan, function(current, processed)
        if not processed and current.KeyCode.Name == Library.Flags[self.flag] then
            self.callback(current.KeyCode.Name)
        end
    end, "Keybind_" .. self.flag)
