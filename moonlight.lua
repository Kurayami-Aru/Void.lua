...
function Auto_Parry.Is_Curved(ball)
    local Ball = Auto_Parry.Get_Ball(ball)
...
do
            ...
            if value then
                Connections_Manager['Auto Parry'] = RunService.PreSimulation:Connect(function()
                    ...
                    for _, Ball in pairs(Balls) do
                        ...
                        local speed_divisor_base = 2.1 + cappedSpeedDiff * 0.002
                        ...
                        local Parry_Accuracy = Ping_Threshold + math.max(Speed / speed_divisor, 8.5)
    ...
    module:create_slider({
        ...
        callback = function(value: boolean)
           Speed_Divisor_Multiplier = 0.75 + (value - 1) * (0.335 / 99)
	end
})
...
if Distance > Spam_Accuracy or Target_Distance > Spam_Accuracy then
    return -- cancela se jÄ‚Æ’Ă‚Â¡ tÄ‚Æ’Ă‚Â¡ longe, evita falso spam
end
