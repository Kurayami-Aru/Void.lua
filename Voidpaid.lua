...
local old_Void = CoreGui:FindFirstChild('Void')

if old_Void then
    Debris:AddItem(old_Void, 0)
end
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('Void/'..file_name..'.json', flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('Void/'..file_name..'.json') then
                self:save(file_name, config)
        
                return
            end
        
            local flags = readfile('Void/'..file_name..'.json')
...
Container.BackgroundColor3 = Color3.fromRGB(230, 220, 255) -- base escura para o gradiente
...
UIStroke.Color = Color3.fromRGB(200, 200, 200) -- borda cinza claro
UIStroke.Transparency = 0.4 -- bem sutil
...
ClientName.Text = 'Void Paid'
ClientName.Name = 'Void Paid'
...
UIGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Color3.fromRGB(240, 240, 240)), -- topo branco
    ColorSequenceKeypoint.new(1, Color3.fromRGB(20, 20, 20))     -- fundo preto
}
UIGradient.Rotation = 90 -- vertical (branco em cima, preto embaixo)
...
Icon.ImageColor3 = Color3.fromRGB(220, 200, 255) -- branco puxado pro roxo
...
Divider.BackgroundColor3 = Color3.fromRGB(80, 60, 130) -- roxo escuro suave
...
Minimize.TextColor3 = Color3.fromRGB(255, 255, 255) -- branco suave
...
UIScale.Parent = Container
    ...
    function self:load()
        ...
        for _, object in Void:GetDescendants() do
...
Options.Parent = Module
            ...
            function ModuleManager:create_paragraph(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>Void</font> user"
                end
            ...
            function ModuleManager:create_text(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>Void</font> user" -- Default rich text
                end
                ...
                function TextManager:Set(new_settings)
                    if not new_settings.rich then
                        ...
                        Body.RichText = true
                        Body.Text = new_settings.richtext or "<font color='rgb(255,0,0)'>Void</font> user" -- Default rich text
                    end
                end;
...
local function UseSwapAbility()
    if Abilities:FindFirstChild("Swap") and Abilities.Swap.Enabled then
        ...
        -- Executa Swap sem mexer na câmera
            ...
            if value then
                Connections_Manager['Auto Parry'] = RunService.PreSimulation:Connect(function()
                    ...
                    for _, Ball in pairs(Balls) do
                        ...
                        local Parry_Accuracy = Ping_Threshold + math.clamp(Speed / speed_divisor, 9.5, Distance - (Speed < 150 and 0.85 or 0.65))
...
Last_Parry = tick()
    ...
    description = 'Manually spams parry at high speed',
    section = 'right',
    callback = function(value: boolean)
        if getgenv().ManualSpamNotify then
            Library.SendNotification({
                title = "Module Notification",
                text = "Manual Spam Parry turned " .. (value and "ON" or "OFF"),
                duration = 3
            })
        end
        ...
        if value then
            Connections_Manager['Manual Spam'] = RunService.Heartbeat:Connect(function()
                ...
                if now - lastManualSpam < (getgenv().ManualSpamCooldown or 0.00005) then return end
                lastManualSpam = now
...
local Module = rage:create_module({
    ...
    callback = (function()
        ...
        local INITIAL_BASE_TRIGGER_DISTANCE = 6.8
        local BASE_TRIGGER_DISTANCE = INITIAL_BASE_TRIGGER_DISTANCE
        local MAX_PLAYER_DISTANCE = 23
        local DELAY_AFTER_TRIGGER = 3
        local MIN_TRIGGER_DISTANCE = 2.0
        local DISTANCE_REDUCTION = 0.3
        ...
        local SpawnedTime = 0
        local LastKnownBall = nil
        local blockTriggerOnSpawnTarget = false

        local handledBalls = {}

        -- Prediction Time dinâmico
                local function getDynamicPredictionTime(speed)
            if speed <= 0 then
                return 1 / 253
        ...
        -- Bloqueio por Infinity ou SingularityCape
        local function isTriggerBlockedBySpecial()
            if getgenv().TriggerbotInfinityDetection and Workspace:FindFirstChild("Infinity") then
                return true
            end

            local root = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
            if root and root:FindFirstChild("SingularityCape") then
                return true
            end
        ...
        local function doParry()
            if AutoClickParried or JustTriggered then return end
        ...
        local function isPlayerMovingBackwards(root, ballVelocity)
            if not root or not ballVelocity then return false end
            local playerVelocity = root.Velocity
            local ballDirection = ballVelocity.Unit
            return playerVelocity:Dot(ballDirection) < 0
        end

        -- Detectar bolas reais spawnadas corretamente
        RunService.RenderStepped:Connect(function()
            for _, ball in pairs(Workspace.Balls:GetChildren()) do
                if ball:GetAttribute("realBall") and not handledBalls[ball:GetDebugId()] then
                    handledBalls[ball:GetDebugId()] = true

                    BallJustSpawned = true
                    SpawnedTime = tick()
                    resetState()

                    local spawnTarget = ball:GetAttribute("target") or "none"
                    if spawnTarget == Player.Name then
                        blockTriggerOnSpawnTarget = true
                    end

                    -- Escuta mudança de target para desbloquear depois
                    ball:GetAttributeChangedSignal("target"):Connect(function()
                        local currentTarget = ball:GetAttribute("target") or "none"
                        if blockTriggerOnSpawnTarget and currentTarget ~= Player.Name then
                            blockTriggerOnSpawnTarget = false
                        end
            ...
            -- Limpeza de bolas que sumiram
            for id, _ in pairs(handledBalls) do
                local stillExists = false
                for _, ball in pairs(Workspace.Balls:GetChildren()) do
                    if ball:GetDebugId() == id then
                        stillExists = true
                        break
                    end
                end
        ...
        Connections_Manager["AutoClick"] = RunService.PreRender:Connect(function()
            local character = Player.Character
            local root = character and character:FindFirstChild("HumanoidRootPart")
            if not root then
                resetState()
                BASE_TRIGGER_DISTANCE = INITIAL_BASE_TRIGGER_DISTANCE
                return
            end

            if tick() - LastTriggerTime < DELAY_AFTER_TRIGGER then
                return
            end
            ...
            for _, Ball in ipairs(Balls) do
                ...
                local lastHit = Ball:GetAttribute("LastHit") or nil
                local zoomies = Ball:FindFirstChild("zoomies")
                if not zoomies then continue end

                local speed = zoomies.VectorVelocity.Magnitude
                local predictionTime = getDynamicPredictionTime(speed)
                local predictedBallPos = Ball.Position + (zoomies.VectorVelocity * predictionTime)
                local distance = (myPos - predictedBallPos).Magnitude

                -- Bloqueio se acabou de spawnar
                if BallJustSpawned and tick() - SpawnedTime < 0.25 then
                    continue
                end

                -- Bloqueio por spawn + alvo sendo você
                if blockTriggerOnSpawnTarget and target == Player.Name then
                    continue
                end

                -- Recuo reduz a distância de trigger
                if target == Player.Name and isPlayerMovingBackwards(root, zoomies.VectorVelocity) then
                    BASE_TRIGGER_DISTANCE = math.max(MIN_TRIGGER_DISTANCE, BASE_TRIGGER_DISTANCE - DISTANCE_REDUCTION)
                else
                    BASE_TRIGGER_DISTANCE = INITIAL_BASE_TRIGGER_DISTANCE
                end
                ...
                if target == Player.Name then
                    ...
                    if canParry and not AutoClickParried and not isTriggerBlockedBySpecial() then
                        doParry()
...
local BallStats = misc:create_module({
    ...
    callback = function(state)
        ...
        if state then
            ...
            -- Remover decorações e efeitos leves
            for _, v in pairs(workspace:GetDescendants()) do
            ...
            -- Desliga a conexão se desativar o módulo
    ...
    local CustomAnnouncer = misc:create_module({
        ...
        callback = function(value: boolean)
            if value then
                ...
                if Winner then
                    Winner.Text = Library._config._flags["announcer_text"] or "discord.gg/Void"
                end
                    ...
                    if Value.Name == "Winner" then
                        Value.Changed:Connect(function(Property)
                            if Property == "Text" and Library._config._flags["Custom_Announcer"] then
                                Value.Text = Library._config._flags["announcer_text"] or "discord.gg/Void"
                            end
                        end)
